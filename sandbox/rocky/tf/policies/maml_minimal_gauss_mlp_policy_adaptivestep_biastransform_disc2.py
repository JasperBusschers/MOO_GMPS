...
class MAMLGaussianMLPPolicy(StochasticPolicy, Serializable):
    ...
    def get_action(self, observation, idx=None):
        ...
        result = [x[0] for x in f_dist([flat_obs])]
        means = np.argmax(result[0] ,axis=1)
        return result, dict(prob=result[0])
    ...
    def get_actions(self, observations):
        ...
        if len(result) == 2:
            ...
            means =np.argmax(result[0],axis=1)# np.array([res for res in result])#[:,0,:]
            #log_stds = np.array([res[1] for res in result])[:,0,:]
        rnd = np.random.normal(size=means.shape)
        actions = means
        return actions, dict(prob=result[0])
