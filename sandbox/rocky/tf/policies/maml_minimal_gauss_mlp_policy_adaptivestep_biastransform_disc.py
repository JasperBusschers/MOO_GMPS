...
from sandbox.rocky.tf.distributions.categorical import  Categorical #diagonal_gaussian import DiagonalGaussian # This is just a util class. No params.
...
# CHANGE
    def dist_info_sym(self, obs_var, state_info_vars=None, all_params=None, is_training=True):
        ...
        mean_var = self._forward(obs_var, all_params, is_training)[0]
        log_std_var=0
        if return_params:
            return dict(prob=mean_var), all_params
        else:
            return dict(prob=mean_var)
...
# CHANGE
    def get_actions(self, observations):
        ...
        if len(result) == 2:
            ...
            means =result[0]# np.array([res for res in result])#[:,0,:]
            #log_stds = np.array([res[1] for res in result])[:,0,:]
        rnd = np.random.normal(size=means.shape)
        actions = means
        return actions, dict(mean=means)
    @property
