...
def plot_pareto(path_pred,path_real, amount, savefile):
    ...
    y_pred = []
    costs = []
    paths = np.load(path_pred,allow_pickle=True).tolist()
    ...
    for i in range(amount):
                ...
                original = joblib.load(open(path_real+str(i)+".pkl", 'rb'))
    ...
    n = [0.05,0.25,0.64,0.73,0.99]  # [i/100 for i in range(0,100,5)]#[0.15,0.2,0.25,0.3,0.35,0.45,0.5, 0.6, 0.85]#[0.15,0.2,0.3,0.45,0.85]#[0.15,0.2,0.25,0.3,0.35,0.45,0.5, 0.6, 0.85]
